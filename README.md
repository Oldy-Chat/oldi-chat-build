# Oldi Chat 0.6.8-beta — кандидат

Рабочий закрытый репозиторий: `Oldy-Chat/oldi-chat-build`.
Основа — сохранённые исходники 0.6.5-beta (versionCode 14) с более свежими
исправлениями CI. Новый кандидат: versionCode 16, пакет `chat.oldy`.
Состояние переноса: [MIGRATION.md](MIGRATION.md).
Изменения текущего кандидата: [RELEASE-0.6.8.md](RELEASE-0.6.8.md).
История предыдущего выпуска: [RELEASE-0.6.7.md](RELEASE-0.6.7.md).

YouTube и генерация/коллекция стикеров используют отдельный сервис на Aéza
`2.56.174.123:9443`. Существующий сервер чата остаётся на прежнем месте.
Установщик нового сервиса не переносит его базы, аккаунты или сообщения.

CI проверяет сервер на временных данных, Android Java/Lint, нативные библиотеки,
собирает неподписанный кандидат и проверяет интерфейс на отдельном эмуляторе.
Кандидат подписывается восстановленным оригинальным ключом вне репозитория.
CI не публикует обновление на старом сервере. Ключи и пользовательские данные
запрещены проверкой исходников и не входят в коммиты.

Для просмотра YouTube используется WebView внутри Oldi с обязательным маршрутом
через Aéza и отдельный встроенный плеер. Внешняя вкладка остаётся только явным
действием для входа Google; её cookies не копируются в WebView. Первый вход Google и системное разрешение Android
на подключение могут потребоваться. Перенос авторизации из приложения YouTube
не выполняется. Успешный HTTP-запрос с VPS не считается проверкой воспроизведения
на телефоне; фактические результаты отмечаются в записи выпуска.

## Локальные проверки

```sh
python3 -m pip install -r tools/ci-python-requirements.txt
python3 tools/check-source-policy.py
python3 tools/ci-server-tests.py
```

Для Android нужны JDK 17, Gradle 8.11.1 и Android SDK 36:

```sh
gradle --no-daemon :app:compileReleaseJavaWithJavac :app:compileReleaseAndroidTestJavaWithJavac :app:lintRelease
```

Это компиляция и статическая проверка; они не подтверждают работу приложения
на телефоне, нативных туннелей, внешних сервисов или обновление установленного APK.

---

## Историческая документация до переноса

Ниже сохранена прежняя документация. Старые ссылки на сборки и команды установки
не описывают результат текущего переноса и не являются инструкцией к развёртыванию.
