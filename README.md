# Oldi Chat 0.6.5 — восстановление исходников

Рабочий закрытый репозиторий: `Oldy-Chat/oldi-chat-build`.
Основа — локальный снимок `bb8d4a4` (0.6.5-beta, versionCode 14).
Состояние переноса и ограничения: [MIGRATION.md](MIGRATION.md).

CI проверяет сервер на временных данных и компиляцию/Android Lint.
CI не подписывает и не публикует APK, не выполняет установку или развёртывание.
Старый сервер и пользовательские данные остаются на месте. Aéza `2.56.174.123`
зарезервирован для будущих YouTube/стикеров и не используется этими проверками.
Сетевое воспроизведение YouTube и генерация стикеров на VPS пока не проверены.

Оригинального Android-ключа подписи в рабочей папке нет. Существующий
`tools/build-apk.sh` требует этот ключ и проверяет прежний сертификат.
Новый ключ взамен старого не создаётся.
