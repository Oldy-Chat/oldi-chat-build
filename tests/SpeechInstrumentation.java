...
public class SpeechInstrumentation extends Instrumentation {
 ...
 public void onStart(){Bundle result=new Bundle();try{Context c=getTargetContext();SpeechModels.Progress progress=new SpeechModels.Progress(){public void status(String s){}public boolean cancelled(){return false;}};
  ...
  for(String language:new String[]{"ru","en"}){String text=WhisperSpeech.recognize(c,fixture("speech/"+language+".wav"),language,progress);check(!text.trim().isEmpty(),"Whisper produced no transcript");String normalized=text.toLowerCase(java.util.Locale.ROOT).replaceAll(language.equals("ru")?"[^а-яё]+":"[^a-z0-9]+"," ").trim();if(language.equals("ru")){String reference=new String(fixture("speech/ru-reference.txt"),java.nio.charset.StandardCharsets.UTF_8).toLowerCase(java.util.Locale.ROOT).replaceAll("[^а-яё]+"," ").trim();double cer=characterError(reference,normalized);check(cer<=0.15,"Whisper Russian reference failed, CER="+cer);result.putString("whisper_russian_cer",Double.toString(cer));}else check(normalized.contains("one")||normalized.contains("1"),"Whisper English reference failed");}
  ...
  result.putString("stream","OLDI_SPEECH_PASS: Bundled Whisper Small Q4 and Vosk baseline checked on Russian neural speech and English human speech; 8/44.1/48 kHz and stereo verified; 601-second file rejected; cancellation verified\n");finish(-1,result);
