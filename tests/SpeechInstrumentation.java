...
public class SpeechInstrumentation extends Instrumentation {
 ...
 public void onStart(){Bundle result=new Bundle();try{Context c=getTargetContext();SpeechModels.Progress progress=new SpeechModels.Progress(){public void status(String s){}public boolean cancelled(){return false;}};
  ...
  File automatic=new File(c.getCacheDir(),"automatic-voice.wav");java.nio.file.Files.write(automatic.toPath(),fixture("speech/ru.wav"));I18n.init(c);Notices.prefs(c).edit().putBoolean("voice_auto_text",true).commit();org.json.JSONObject meta=new org.json.JSONObject();SpeechAuto.annotate(c,()->false,automatic,meta);check(!meta.optString("transcript").isEmpty(),"Automatic outgoing voice text missing");org.json.JSONObject cancelledMeta=new org.json.JSONObject();SpeechAuto.annotate(c,()->true,automatic,cancelledMeta);check(!cancelledMeta.has("transcript"),"Changed account received a transcript");automatic.delete();
  ...
  result.putString("stream","OLDI_SPEECH_PASS: Automatic outgoing voice text and account cancellation verified; Russian neural sample and English human speech recognized by on-device Vosk against published references; 8/44.1/48 kHz and stereo verified; 601-second file rejected; cancellation verified\n");finish(-1,result);
