...
Large files require `tools/update-cloud-files.py` on the **existing Cloud
5.42.102.11**, not Aéza. It patches only the known upload limit and attachment
metadata allowlist, preserves newer unrelated code, backs up the previous
...
speech recognition, independent video peers, post-reboot authenticated polling,
