...
PUBLISHER
archive_name='OldyChat-0.6.7-server.tar.gz'
...
with tempfile.TemporaryDirectory(prefix='oldy-update-') as tmp:
 ...
 if bundle is None:
  print('Скачиваем OldЫ Chat 0.6.7…',flush=True);downloaded=folder/'release.zip'
 ...
 expected={'OldyChat-latest.apk','server.py','account_features.py','install.sh','configure-mail.py','disk_storage.py','configure-disk.py','legal_service.py','legal_texts.json','configure-legal.py','retention.py','sticker_generation.py','configure-stickers.py','sticker_diagnostics.py','release.json'}
 ...
 if manifest['package']!='chat.oldy' or manifest['version_code']!=16 or manifest['sha256']!=hashlib.sha256(apk.read_bytes()).hexdigest() or manifest['size']!=apk.stat().st_size:raise SystemExit('Проверка APK не пройдена. Сервер не изменён.')
 ...
 print('OLDY CHAT: UPDATE AVAILABLE 0.6.7-beta',flush=True)
...
ast.parse(script)
args.output.write_text("python3 - <<'OLDY_UPDATE_067'\n"+script+"OLDY_UPDATE_067\n")
