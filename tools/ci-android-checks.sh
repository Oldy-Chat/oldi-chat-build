...
conversation_result=$?
timeout 180s adb shell am instrument -w chat.oldy.tests/chat.oldy.SpeechInstrumentation | tee build/ui-report/speech.txt
...
test "$ui_result" -eq 0 && test "$conversation_result" -eq 0 && test "$speech_result" -eq 0 && test "$media_result" -eq 0
