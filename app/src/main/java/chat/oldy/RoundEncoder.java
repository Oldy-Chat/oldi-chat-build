...
final class RoundEncoder {
 ...
 RoundEncoder(File target,EglBase.Context shared)throws Exception{
  ...
  mic.startRecording();videoThread=new Thread(()->videoLoop(),"oldy-round-video");audioThread=new Thread(()->audioLoop(),"oldy-round-audio");videoThread.start();audioThread.start();}catch(SecurityException error){recording=false;cleanup();file.delete();throw new IOException(I18n.t("Разреши доступ к микрофону для записи кружка."),error);}catch(Exception error){recording=false;cleanup();file.delete();throw error;}
 }
