...
final class SpeechNoteView extends LinearLayout {
 ...
 void update(){String saved=SpeechNotes.saved(owner,mid);SpeechNotes.Job work=SpeechNotes.job;boolean busy=work!=null&&work.account.equals(owner.nick())&&work.mid.equals(mid);progress.setVisibility(busy?VISIBLE:GONE);boolean tooLong=message.optLong("duration")>600000;button.setEnabled(!tooLong);button.setAlpha(tooLong?.6f:1f);button.setText(tooLong?SpeechDecoder.limit():busy?work.status+" · "+LegalCenter.tr("Отмена","Cancel"):saved.isEmpty()?LegalCenter.tr("Aa  В текст · до 10 минут","Aa  To text · up to 10 minutes"):LegalCenter.tr(expanded?"Aa  Свернуть текст":"Aa  Развернуть текст",expanded?"Aa  Collapse text":"Aa  Expand text"));transcript.setVisibility(!saved.isEmpty()?VISIBLE:GONE);transcript.setMaxLines(expanded?Integer.MAX_VALUE:4);transcript.setText(saved);}
