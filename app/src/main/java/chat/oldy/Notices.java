...
final class Notices {
 ...
 static void show(Context c,Vault v,JSONObject m,JSONObject user){
  ...
  boolean preview=prefs(c).getBoolean("preview",false);boolean audible=rule.sound&&!m.optBoolean("silent"),vibration=rule.vibrate&&!m.optBoolean("silent");String ch=channelId(c,audible,vibration);
  NotificationManager nm=c.getSystemService(NotificationManager.class);NotificationChannel channel=new NotificationChannel(ch,I18n.t("Сообщения")+(audible?" · "+I18n.t(TONE_NAMES[tone(c)]):I18n.t(" · без звука")),NotificationManager.IMPORTANCE_HIGH);
  channel.setSound(audible?Uri.parse("android.resource://"+c.getPackageName()+"/"+TONES[tone(c)]):null,new AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_NOTIFICATION).build());if(vibration)channel.setVibrationPattern(new long[]{0,80,70,80});channel.enableVibration(vibration);nm.createNotificationChannel(channel);
