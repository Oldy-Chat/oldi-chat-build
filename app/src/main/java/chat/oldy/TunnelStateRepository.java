...
final class TunnelStateRepository {
 ...
 static void refresh(){if(context!=null)try{java.io.File f=new java.io.File(context.getFilesDir(),"youtube-state");if(f.length()>1024)return;String[] parts=new String(java.nio.file.Files.readAllBytes(f.toPath()),java.nio.charset.StandardCharsets.UTF_8).split("\n",-1);state=parts[0];heartbeat=Long.parseLong(parts[1]);if(parts.length>2)error=parts[2];if(parts.length>4){txBytes=Long.parseLong(parts[3]);rxBytes=Long.parseLong(parts[4]);}if(parts.length>7){webPort=Integer.parseInt(parts[5]);webConnections=Long.parseLong(parts[6]);webRx=Long.parseLong(parts[7]);}}catch(Exception ignored){}}
