...
final class TunnelStateRepository {
 static volatile String state="OFF",error="";static volatile long heartbeat,txBytes,rxBytes;static Context context;static volatile int webPort;static volatile long webConnections,webRx,webRejected;static volatile String webError="";
 ...
 static void set(String next){state=next;heartbeat=SystemClock.elapsedRealtime();if(context!=null)try{java.io.File dir=context.getFilesDir(),temp=new java.io.File(dir,"youtube-state-"+android.os.Process.myPid()+".tmp"),target=new java.io.File(dir,"youtube-state");String data=next+"\n"+heartbeat+"\n"+error+"\n"+txBytes+"\n"+rxBytes+"\n"+webPort+"\n"+webConnections+"\n"+webRx+"\n"+webRejected+"\n"+webError;java.nio.file.Files.write(temp.toPath(),data.getBytes(java.nio.charset.StandardCharsets.UTF_8));java.nio.file.Files.move(temp.toPath(),target.toPath(),java.nio.file.StandardCopyOption.REPLACE_EXISTING,java.nio.file.StandardCopyOption.ATOMIC_MOVE);}catch(Exception ignored){}}
 static void refresh(){if(context!=null)try{java.io.File f=new java.io.File(context.getFilesDir(),"youtube-state");if(f.length()>1024)return;String[] parts=new String(java.nio.file.Files.readAllBytes(f.toPath()),java.nio.charset.StandardCharsets.UTF_8).split("\n",-1);state=parts[0];heartbeat=Long.parseLong(parts[1]);if(parts.length>2)error=parts[2];if(parts.length>4){txBytes=Long.parseLong(parts[3]);rxBytes=Long.parseLong(parts[4]);}if(parts.length>7){webPort=Integer.parseInt(parts[5]);webConnections=Long.parseLong(parts[6]);webRx=Long.parseLong(parts[7]);}if(parts.length>9){webRejected=Long.parseLong(parts[8]);webError=parts[9];}}catch(Exception ignored){}}
