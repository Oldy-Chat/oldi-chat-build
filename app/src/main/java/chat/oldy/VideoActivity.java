...
public class VideoActivity extends Activity {
 ...
 public void onCreate(Bundle b){
  ...
  controls=new LinearLayout(this);controls.setOrientation(android.widget.LinearLayout.VERTICAL);controls.setPadding(dp(12),0,dp(12),dp(8));controls.setBackground(new GradientDrawable(GradientDrawable.Orientation.BOTTOM_TOP,new int[]{0xcc080d16,0x00080d16}));
 ...
 void showSettings(){touch();setControls(true);LinearLayout list=new LinearLayout(this);list.setOrientation(android.widget.LinearLayout.VERTICAL);list.setPadding(dp(16),dp(8),dp(16),dp(16));list.setBackgroundColor(0xff151d2c);AlertDialog dialog=new AlertDialog.Builder(this,android.R.style.Theme_Material_Dialog_NoActionBar).setView(list).create();
  ...
  public long open(DataSpec spec)throws IOException{
   ...
   try{
    if(id==null){byte[] data=MediaFiles.bytes(context,local);if(spec.position>data.length)throw new EOFException();left=spec.length==C.LENGTH_UNSET?data.length-spec.position:Math.min(spec.length,data.length-spec.position);input=new ByteArrayInputStream(data,(int)spec.position,(int)left);uri=spec.uri;}
     ...
     uri=Uri.parse(base+"/video-stream/"+id+(quality>0?"?quality="+quality:compatible?"?compatible=1":""));connection=(HttpsURLConnection)new URL(uri.toString()).openConnection(java.net.Proxy.NO_PROXY);connection.setSSLSocketFactory(api.factory());connection.setInstanceFollowRedirects(false);connection.setConnectTimeout(15000);connection.setReadTimeout(20000);connection.setRequestProperty("Authorization","Bearer "+ChatService.vault(context).token());connection.setRequestProperty("Accept-Encoding","identity");connection.setRequestProperty("Range","bytes="+spec.position+"-"+(spec.length==C.LENGTH_UNSET?"":Long.toString(spec.position+spec.length-1)));
