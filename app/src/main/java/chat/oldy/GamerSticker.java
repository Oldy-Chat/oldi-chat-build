...
final class GamerSticker extends View {
 ...
 protected void onDraw(Canvas c){float z=Math.min(getWidth()/140f,getHeight()/140f);c.save();c.translate((getWidth()-140*z)/2,(getHeight()-140*z)/2);c.scale(z,z);
  ...
  color(0xff374269);c.drawRoundRect(38,20,103,47,16,16,p);color(0xff202a45);c.drawRoundRect(28,39,108,50,5,5,p);color(0xff5cf0e1);p.setTypeface(Typeface.create("monospace",android.graphics.Typeface.BOLD));p.setTextSize(13);c.drawText("OLDY",52,37,p);
  ...
  color(0xff101b30);c.drawRoundRect(3,109,137,136,9,9,p);color(0xffeffaff);p.setTextAlign(Paint.Align.CENTER);p.setTypeface(Typeface.create("sans-serif-condensed",android.graphics.Typeface.BOLD));String caption=I18n.t(CAPTIONS[style]);p.setTextSize(13);if(p.measureText(caption)>124)p.setTextSize(13*124/p.measureText(caption));c.drawText(caption,70,127,p);p.setTextAlign(Paint.Align.LEFT);c.restore();if(isShown()&&Notices.prefs(getContext()).getBoolean("animations",true))postInvalidateDelayed(100);
 }
}
