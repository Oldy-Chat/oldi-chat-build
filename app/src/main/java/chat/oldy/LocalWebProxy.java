...
final class LocalWebProxy implements Closeable {
 ...
 boolean owned(Socket socket){if(Build.VERSION.SDK_INT<29)return false;try{ConnectivityManager cm=service.getSystemService(ConnectivityManager.class);int owner=cm.getConnectionOwnerUid(OsConstants.IPPROTO_TCP,(InetSocketAddress)socket.getRemoteSocketAddress(),(InetSocketAddress)socket.getLocalSocketAddress());return owner==android.os.Process.myUid()||(!service.browserPackage.isEmpty()&&owner==service.getPackageManager().getApplicationInfo(service.browserPackage,0).uid);}catch(Exception denied){return false;}}
