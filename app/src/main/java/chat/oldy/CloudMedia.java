...
final class CloudMedia {
 ...
 static void attach(MainActivity a,Uri uri,String mime,String name,String target,String caption,JSONObject extra)throws Exception{
  Vault vault=a.vault;String vid="";boolean queued=false;JSONObject body=MediaFiles.importFile(a,uri,mime,name);String key=body.getString("local");if(extra!=null)for(String field:new String[]{"duration","waveform","transcript","transcript_language","custom_sticker","sticker_id","sticker_author"})if(extra.has(field))body.put(field,extra.get(field));
  try{
